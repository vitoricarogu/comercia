  ...
  useEffect(() => {
    const loadDashboardData = async () => {
      try {
        ...
        // Carregar dados reais do backend com tratamento de erro
        const [agentStats, conversationStats, agents, conversations] = await Promise.allSettled([
        ...
        // Processar resultados com fallback para dados vazios
        const agentStatsData = agentStats.status === 'fulfilled' && agentStats.value.success 
          ? agentStats.value.data?.stats 
          : { total: 0, active: 0 };
          
        const conversationStatsData = conversationStats.status === 'fulfilled' && conversationStats.value.success 
          ? conversationStats.value.data?.stats 
          : { total: 0, active: 0, avgSatisfaction: 0, avgResponseTime: 0 };
          
        const agentsData = agents.status === 'fulfilled' && agents.value.success 
          ? agents.value.data?.agents || [] 
          : [];
          
        const conversationsData = conversations.status === 'fulfilled' && conversations.value.success 
        ...
        const dashboardStats = {
          overview: {
            totalAgents: agentStatsData.total || 0,
            activeAgents: agentStatsData.active || 0,
            totalConversations: conversationStatsData.total || 0,
            activeConversations: conversationStatsData.active || 0,
            avgSatisfaction: conversationStatsData.avgSatisfaction || 0,
            avgResponseTime: conversationStatsData.avgResponseTime || 0
          },
          trends: {
            dailyConversations: conversationStatsData.dailyConversations || [],
            dailyAgents: agentStatsData.dailyCreated || []
          }
        };
